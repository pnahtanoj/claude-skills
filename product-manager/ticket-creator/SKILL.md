...
- **Add a 🧪 Testing Checkpoint section when a ticket closes a vertical slice.** The signal: completing this ticket means two or more prior tickets can be validated end-to-end by a human for the first time. Check the dependency graph — if this ticket is a convergence point where multiple upstream threads meet, it's a checkpoint. Describe what to test and what a passing run looks like. Do not add this section to tickets that don't represent a meaningful integration milestone.
